...
Rustではデータの変更を行う際に、意図しないバグを防ぐためにいくつかのルールが設けられています。このルールはデータ競合を防ぐために存在し、上述したエラーが発生する理由についても、これから解説するルールを読み解けば理解することができます。ルールは以下のとおりです：

- 不変参照が存在している間はデータの変更をしてはならない
- 「可変参照が一つだけ存在する」もしくは「複数個の不変参照が存在する」のどちらかが許可される
- 可変参照が存在している間は可変参照を通してのみデータを変更できる

{{% notice info "MEMO" %}}
上記ルール中で「参照が存在する」という言い回しをしています。シンプルに読むと「参照が存在する」とは参照変数が生きている間、つまり関数やブロックのスコープ内は参照が存在しているという理解をしたくなると思います。しかし、ここで言う「参照が存在する」とは「参照が作られた時から最後に使われた時まで」を指します。つまり参照が存在する範囲は関数やブロックのスコープよりも短くなる可能性があります。
{{% /notice %}}

それでは上記ルールについて解説していきます。

#### ルール1.「可変参照が一つだけ存在する」もしくは「複数個の不変参照が存在する」のどちらかが許可される
...
このコードは問題なく動作し"10 * 2 = 20"と表示されます。参照_ref_aが宣言されていますが、エラーにはなりません。これは最初に書いた前提を見るとわかります。参照は最後に使われた時まで生存しています。しかし_ref_aは宣言してから一度も使われていません。

つまり宣言と同時に参照は死んでしまうのでa = a * 2を実行する時点で参照は存在していないことになります。よって「参照が存在している場合、参照元のデータを変更してはならない」が守られているため、このコードはビルドできます。
...
戻り値をaではなく、*_ref_aを返すようにしました。_ref_aはaを参照しているので、aと同じ値を見ているはずです。よって問題なく動作するように見えます。しかしこのコードはビルドエラーとなります。参照_ref_aは関数の最後まで生存することになり「参照が存在している場合、参照元のデータを変更してはならない」に違反しているためです。

#### ルール2.不変参照が存在している間はデータの変更をしてはならない

一つの値に対する可変参照は一つしか存在できません。ルール1ではデータ変更する操作を記述するとビルドエラーになりましたが、このルールでは変更操作を記述しなくても可変参照が二つ以上存在した時点でエラーになります。
...
fn double(x: i32) -> i32 {
    ...
    let _ref_a_1 = &mut a;
    let _ref_a_2 = &mut a;
    let ret = (*_ref_a_1) + (*_ref_a_2);
    ret
}
```

上記コードはaに対する可変参照が二つ存在しているためエラーとなります。

{{< i prefix="3-" method="resize" width=1920 >}}


#### 3.可変参照が存在している間は可変参照を通してのみデータを変更できる

Stringと&strの違いについての解説を書いたので見てほしいです。以下の内容になります。
### Rustにおける文字列型：Stringと&strについて

少し話題が変わってしまうのですが、ここでRustにおける文字列について話しておこうと思います。この後に解説するライフタイムについてのサンプルコード内で&strを使用しているので、このタイミングでStringと&strについての違いなど、説明を入れておく必要があると感じたためです。

これまで出てきたサンプルコード内でも文字列を扱ってきましたが、その型はStringのみでした。Rustプログラミングにおいては文字列を扱う際に主にStringと&strを使用することになります。

{{% notice tip "TIP" %}}
もちろんstr型も存在していますが使うことは基本的にありません。その理由も後述しています。
{{% /notice %}}

#### Stringについて

Stringは可変文字列の型です。可変なので内部状態を変更することができます。これまで見てきたサンプルコードではpush_strを使って、文字列を追記して状態を変更していました。Stringは実際の文字列データ自体は保持していません。Stringが保持するのは以下の情報です。

- 実際の文字列データへのポインタ
- 文字列の長さ(len)
- 文字列が確保しているメモリ領域の容量(capacity)

Stringは実際の文字列データへのポインタを持っていますが、このポインタが指し示す文字列データがstrです。

#### &strについて

strが何かについてはすでにStringについての解説内で書きました。Stringが内部に持っているポインタが指し示す先の実体がstrです。つまり&strはstrの参照である、ということになります。Rustにおける文字列操作でstrを使うことはないと書きました。それはstrが事前にサイズが決まらない型であるからです。

i32やstructや固定長の配列などは、すべて事前にサイズが確定しています。よってコンパイル時に必要なメモリ領域を確定できます。しかしstrは事前にサイズがわからないため直接使うことはできないので、参照である&strを通して使うことになります。
