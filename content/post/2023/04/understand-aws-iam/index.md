...
実際の作業に入る前に、どのようにリソース(ユーザー、グループ、ロール、ポリシー)を作成するべきなのか洗い出しを行なっていきます。
...
では実際にポリシーを記述していきます。まずはdeveloper-baseで使用するポリシーから作成します。
...
ポリシーの定義が完了したので、ユーザー、グループ、ロールを作成しポリシーの割り当て(アタッチ)を行います。

### グループの作成

ユーザーがグループに所属する必要があるため先にグループを作成します。
...
まずは作成したユーザでログインを行なってみます。

IAMダッシュボードを表示してみます。MFAの設定とアクセスキーの作成が行えることが確認できます(ここでは省略しますがMFAは必ず設定しましょう)。そして、それ以外の操作は行えないことが確認できます。

続いてS3でバケットを作成してみます。

S3を操作するLambda関数を作成して実行してみましょう。

最後に、ロールをスイッチしてIAMの全ての操作が行えることを確認します。
