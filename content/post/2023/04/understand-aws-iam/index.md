...
作成すべきポリシーを洗い出すために、どのようなユーザ/グループ/ロールが必要になるかを検討し、そこからポリシーを逆算していきます。
...
以降で具体的なポリシーの定義を実装します。

#### enable-mfa

MFAを設定可能にするポリシーです。ルートアカウントだけでなくログイン操作をする全てのユーザはMFAを設定できるようにします。
...
`<role-name>`には委譲を許可するロール名を指定します。ここではlambda-execを指定します。
lambda-execロールはこの後の工程で作成します。

#### enable-list-roles

ロールの一覧表示を可能にします。コンソール画面でロールの一覧から選択する際に使用します。
...
access-analyzer:ValidatePolicyはポリシーに問題がある際にポリシーエディタ上で警告を表示してくれます。
無くても支障はありませんが、警告は有益な情報となるので追加しておきます。
...
バケットの作成権限はないので、マネージメントコンソールから手動で作成しておく前提になります。
